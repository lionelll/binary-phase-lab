...
标题内容垂直居中，标题栏宽度为 `100%`，底部分隔线沿用全局 `--line`，背景保持透明。所有侧栏标题统一采用晶体结构的 `17px / 800 / 54px` 标准。“相图类型”“典型合金预设”“功能模块”“辅助显示”，以及右栏“当前信息”“相平衡”“显微组织及相对含量”“反应信息”标题均不显示前置图标。原“实验参数”面板改名为“合金成分与温度控制”，作为子区块并入“功能模块”面板。左栏面板顺序固定为：相图类型 → 典型合金预设 → 功能模块（含合金成分与温度控制）→ 辅助显示。
...
| `--lab-control-label-size` | `13px` | 合金成分与温度控制名称及当前值；对齐三元相图控制标签 |
...
| `--lab-balance-meta-size` | `13px` | 相平衡与组织相对含量卡中的说明、名称和百分比 |
| `--lab-balance-value-size` | `15px` | 相平衡相名称与当前显微组织名称 |
| `--lab-reaction-title-size` | `15px` | 非激活状态的反应信息首行；对齐晶体结构教学正文 |
| `--lab-reaction-detail-size` | `15px` | 反应说明和三相提示正文；对齐晶体结构教学正文 |

补充规则：相图选择卡片只显示小型相图图标与相图名称，不显示体系说明或右侧箭头。四种相图统一使用 Lucide 白色线性相图图标；普通与选中状态均保持纯白色。选择卡片和功能模块的高度、内边距、图标尺寸、文字间距、圆角与列表留白均严格采用晶体结构侧栏的计算值。左侧“功能模块”只保留“杠杆定律”和“三相反应”；相图结构是中央画布的常驻能力，冷却过程由顶栏“自动凝固”直接启动。“典型合金预设”在桌面端和移动端均可通过标题栏折叠，首次进入页面时两端均默认折叠。卡片标题统一使用 `line-height: 1.35`。当前信息标题不显示前置图标，标题栏、两列比例、内容内边距、每行高度、字号、字重、颜色与标点均严格采用晶体结构当前信息卡标准；相区值的蓝色属于业务强调，继续保留。“显微组织及相对含量”固定置于右栏“相平衡”下方，其比例条复用相平衡的字号、颜色和轨道样式。“反应信息”不保留空状态占位，仅当当前成分和温度命中适用的三相反应线时在右栏显示；离开反应线后立即隐藏。反应信息内容区采用晶体结构教学卡的正文层级；三相反应被激活时的 `18px` 公式属于业务强调状态，继续保留，不降为普通正文字号。
