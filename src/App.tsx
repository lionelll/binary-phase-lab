...
export default function App(){
  ...
  const [display,setDisplay]=useState<DisplayOptions>({labels:true,constituents:false,keyPoints:true,tieLine:true});
  ...
  const reset=()=>{cooling.reset();setComposition(diagram.defaultState.composition);setTemperature(diagram.defaultState.temperature);setModule('structure');setDisplay({labels:true,constituents:false,keyPoints:true,tieLine:true});setCoolingInvariant(null)};
  ...
  const displayedState=coolingInvariant?{...state,kind:'invariant' as const,invariant:coolingInvariant,regionLabel:'三相平衡',phases:coolingInvariant.phaseCompositions.map(item=>item.phase),equilibrium:coolingInvariant.phaseCompositions.map(item=>({...item,fraction:0}))}:state;
