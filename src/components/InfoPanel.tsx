...
import { isVisibleFraction } from '../lib/fractionDisplay';
import { describeMicrostructureSnapshot } from '../lib/microstructure';
import { phaseStateDisplayLabel } from '../lib/phaseLabel';
...
export function InfoPanel({diagram,state,module}:{diagram:PhaseDiagramDefinition;state:PhaseState;module:ModuleId}){
  const visibleEquilibrium=state.equilibrium.filter((item)=>isVisibleFraction(item.fraction));
  const [left,right]=visibleEquilibrium.length===2?visibleEquilibrium:[];
  const precision=diagram.compositionAxis.max<=10?3:1;
  const microstructureSnapshot=describeMicrostructureSnapshot(diagram,state.composition,state.temperature,state);
  const {microstructure,fractions:microstructureFractions}=microstructureSnapshot;
  const currentPhaseLabel=phaseStateDisplayLabel(diagram,state);
